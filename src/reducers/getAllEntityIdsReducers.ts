...
import { GET_ALL_ENTITY_REQUEST, GET_ALL_ENTITY_SUCCESS, GET_ALL_ENTITY_ERROR } from '../actions/actionsTypes'
...
export default (state: EntityIdsStore = undefined, action: GetAllEntityTypes) => {
    ...
    switch(action.type) {
        case GET_ALL_ENTITY_REQUEST: return Object.assign({}, state, { status: Status.Requested })
        case GET_ALL_ENTITY_SUCCESS: {
            const dictionary = new Map(state.dictionary)
            dictionary.set(action.entity.name, action.entitiesIds)
